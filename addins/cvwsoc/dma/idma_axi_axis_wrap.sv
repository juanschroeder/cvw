...
module idma_axi_axis_wrap #(
  ...
  parameter bit          EnableDesc64AxiAxis  = 1'b1,
  ...
  input  logic [$clog2(AxisFifoCapacityBytes):0] axis_fifo_occupancy_i,
  output logic                   irq_o,
  ...
  localparam int unsigned DmaBufferDepth = 8;
  ...
  localparam int unsigned AxisBeatBytes = AxiDataWidth / 8;

  initial begin
    assert (AxiDataWidth >= 8 && (AxiDataWidth % 8) == 0)
      else $fatal(1, "idma_axi_axis_wrap: AxiDataWidth must be a positive byte multiple");
    assert (!EnableAxisFifoAdmission || AxisFifoCapacityBytes > AxisFifoSafetyMarginBytes)
      else $fatal(1, "idma_axi_axis_wrap: FIFO capacity must exceed its safety margin");
    assert (!EnableAxisFifoAdmission || AxisFifoSafetyMarginBytes >= AxisBeatBytes)
      else $fatal(1, "idma_axi_axis_wrap: FIFO safety margin must be at least one AXI beat");
    assert (!EnableAxisFifoAdmission || (AxisFifoCapacityBytes % AxisBeatBytes) == 0)
      else $fatal(1, "idma_axi_axis_wrap: FIFO capacity must be an AXI-beat multiple");
  end
  ...
  (* mark_debug = "true" *) logic [15:0] dbg_axis_stall_cycles;
  (* mark_debug = "true" *) logic [15:0] dbg_axi_ar_stall_cycles;
  (* mark_debug = "true" *) logic [15:0] dbg_axi_r_stall_cycles;
  ...
  if (EnableDesc64) begin : gen_desc64
    ...
    axi_to_reg_v2 #(
      ...
      .clk_i,
      .rst_ni,
      .axi_req_i ( axi_slv_req_i[Desc64MemIdx] ),
      .axi_rsp_o ( axi_slv_rsp_o[Desc64MemIdx] ),
      .reg_req_o ( desc_reg_req                 ),
      .reg_rsp_i ( desc_reg_rsp                 )
    );

    idma_desc64_top #(
    //idma_desc64_axi_axis_top #(
      .AddrWidth        ( 64                  ),
      .DataWidth        ( AxiDataWidth        ),
      .AxiIdWidth       ( AxiIdWidth          ),
      .idma_req_t       ( idma_req_t          ),
      //.idma_req_metadata_t( idma_desc64_axi_axis_pkg::idma_req_metadata_t ),
  ...
  if (EnableDesc64AxiAxis) begin : gen_desc64_axiaxis
    desc_regs_req_t axis_desc_reg_req;
    desc_regs_rsp_t axis_desc_reg_rsp;
    desc_regs_req_t axis_desc_idma_reg_req;
    desc_regs_rsp_t axis_desc_idma_reg_rsp;
    ...
    logic [AxisOutstandingWidth-1:0] axis_outstanding_q;
    logic axis_stream_in_flight_q;
    logic axis_desc_is_stream;
    logic axis_desc_admissible;
    logic axis_desc_accept;
    logic axis_desc_reject;
    logic axis_desc_reject_accept;
    logic axis_desc_backend_response;
    logic axis_reject_pending_q;
    idma_rsp_t axis_reject_rsp;
    logic [TfLenWidth:0] axis_desc_rounded_bytes;
    logic [TfLenWidth:0] axis_desc_required_bytes;
    logic [TfLenWidth:0] axis_fifo_free_bytes;
    ...
    axi_to_reg_v2 #(
      ...
      .clk_i,
      .rst_ni,
      .axi_req_i ( axi_slv_req_i[Desc64AxisIdx] ),
      .axi_rsp_o ( axi_slv_rsp_o[Desc64AxisIdx] ),
    ...
    idma_desc64_axi_axis_top #(
      ...
      .axi_ar_id_i       ( '0                  ),
      .axi_aw_id_i       ( '0                  ),
      .slave_req_i       ( axis_desc_idma_reg_req ),
      .slave_rsp_o       ( axis_desc_idma_reg_rsp ),
      //.idma_req_o        ( axis_desc_req_raw    ),
      .idma_req_o        ( axis_desc_req        ),
      ...
      .idma_busy_i       ( |busy                ),
      .stop_i            ( axis_cyclic_stop_wr  ),
      .running_o         ( axis_cyclic_running  ),
      .stopped_o         ( axis_cyclic_stopped  ),
      .irq_o             ( desc64_axis_irq_pulse )
    );

    // Regbus retains the low bits of the 0x1008_2000 system address.
    localparam logic [15:0] AxisIrqStatusOffset = 16'h2100;
    localparam logic [15:0] AxisIrqEnableOffset = 16'h2104;
    localparam logic [15:0] AxisCyclicCtrlOffset = 16'h2110;
    localparam int unsigned AxisRegBytes = AxiDataWidth / 8;
    localparam int unsigned AxisRegByteOffWidth = $clog2(AxisRegBytes);
    localparam logic [15:0] AxisRegByteMask = 16'(AxisRegBytes - 1);
    localparam logic [15:0] AxisIrqStatusWordOffset = AxisIrqStatusOffset & ~AxisRegByteMask;
    localparam logic [15:0] AxisIrqEnableWordOffset = AxisIrqEnableOffset & ~AxisRegByteMask;
    localparam logic [15:0] AxisCyclicCtrlWordOffset = AxisCyclicCtrlOffset & ~AxisRegByteMask;
    localparam int unsigned AxisIrqStatusLane = AxisIrqStatusOffset % AxisRegBytes;
    localparam int unsigned AxisIrqEnableLane = AxisIrqEnableOffset % AxisRegBytes;
    localparam int unsigned AxisCyclicCtrlLane = AxisCyclicCtrlOffset % AxisRegBytes;

    logic axis_sel_irq_status_word;
    logic axis_sel_irq_enable_word;
    logic axis_sel_irq_status_lane;
    logic axis_sel_irq_enable_lane;
    logic axis_sel_irq_status;
    logic axis_sel_irq_enable;
    logic axis_sel_irq;
    logic axis_irq_wbit;
    logic axis_irq_wstrb;
    logic axis_irq_clear_wr;
    logic axis_irq_enable_wr;
    logic axis_irq_enable;
    logic axis_irq_pending;
    logic axis_sel_cyclic_ctrl;
    logic axis_cyclic_stop_wr;
    logic axis_cyclic_running;
    logic axis_cyclic_stopped;
    logic axis_stop_draining_q;

    assign axis_sel_irq_status_word =
        axis_desc_reg_req.valid &&
        ((axis_desc_reg_req.addr[15:0] & ~AxisRegByteMask) == AxisIrqStatusWordOffset);
    assign axis_sel_irq_enable_word =
        axis_desc_reg_req.valid &&
        ((axis_desc_reg_req.addr[15:0] & ~AxisRegByteMask) == AxisIrqEnableWordOffset);
    assign axis_sel_irq_status_lane =
        axis_desc_reg_req.write ? axis_desc_reg_req.wstrb[AxisIrqStatusLane] :
                                  (axis_desc_reg_req.addr[AxisRegByteOffWidth-1:0] ==
                                   AxisRegByteOffWidth'(AxisIrqStatusLane));
    assign axis_sel_irq_enable_lane =
        axis_desc_reg_req.write ? axis_desc_reg_req.wstrb[AxisIrqEnableLane] :
                                  (axis_desc_reg_req.addr[AxisRegByteOffWidth-1:0] ==
                                   AxisRegByteOffWidth'(AxisIrqEnableLane));
    assign axis_sel_irq_status = axis_sel_irq_status_word && axis_sel_irq_status_lane;
    assign axis_sel_irq_enable = axis_sel_irq_enable_word && axis_sel_irq_enable_lane;
    assign axis_sel_irq = axis_sel_irq_status || axis_sel_irq_enable;
    assign axis_sel_cyclic_ctrl = axis_desc_reg_req.valid &&
        ((axis_desc_reg_req.addr[15:0] & ~AxisRegByteMask) == AxisCyclicCtrlWordOffset) &&
        (axis_desc_reg_req.write ? axis_desc_reg_req.wstrb[AxisCyclicCtrlLane] :
         (axis_desc_reg_req.addr[AxisRegByteOffWidth-1:0] ==
          AxisRegByteOffWidth'(AxisCyclicCtrlLane)));
    assign axis_cyclic_stop_wr = axis_sel_cyclic_ctrl && axis_desc_reg_req.write &&
        axis_desc_reg_req.wstrb[AxisCyclicCtrlLane] &&
        axis_desc_reg_req.wdata[AxisCyclicCtrlLane*8];
    assign axis_irq_wbit =
        axis_sel_irq_enable ? axis_desc_reg_req.wdata[AxisIrqEnableLane*8] :
                              axis_desc_reg_req.wdata[AxisIrqStatusLane*8];
    assign axis_irq_wstrb =
        axis_sel_irq_enable ? axis_desc_reg_req.wstrb[AxisIrqEnableLane] :
                              axis_desc_reg_req.wstrb[AxisIrqStatusLane];
    assign axis_irq_clear_wr =
        axis_sel_irq_status && axis_desc_reg_req.write && axis_irq_wstrb && axis_irq_wbit;
    assign axis_irq_enable_wr =
        axis_sel_irq_enable && axis_desc_reg_req.write && axis_irq_wstrb;

    always_comb begin
      axis_desc_idma_reg_req = axis_desc_reg_req;
      axis_desc_idma_reg_req.valid = axis_desc_reg_req.valid &&
                                     !axis_sel_irq && !axis_sel_cyclic_ctrl;

      axis_desc_reg_rsp = axis_desc_idma_reg_rsp;
      if (axis_sel_irq || axis_sel_cyclic_ctrl) begin
        axis_desc_reg_rsp.ready = 1'b1;
        axis_desc_reg_rsp.error = 1'b0;
        axis_desc_reg_rsp.rdata = '0;

        if (!axis_desc_reg_req.write) begin
          unique case (1'b1)
            axis_sel_cyclic_ctrl: begin
              // STOP is write-one command-only; status occupies bits 2:1.
              axis_desc_reg_rsp.rdata[AxisCyclicCtrlLane*8 + 1] = axis_cyclic_running;
              axis_desc_reg_rsp.rdata[AxisCyclicCtrlLane*8 + 2] = axis_cyclic_stopped;
            end
            axis_sel_irq_status:
              axis_desc_reg_rsp.rdata = {{(AxiDataWidth-1){1'b0}}, axis_irq_pending};
            axis_sel_irq_enable:
              axis_desc_reg_rsp.rdata[AxisIrqEnableLane*8] = axis_irq_enable;
            default: axis_desc_reg_rsp.rdata = '0;
          endcase
        end
      end
    end
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
      if (!rst_ni) begin
        axis_irq_enable <= 1'b0;
        axis_irq_pending <= 1'b0;
      end else begin
        if (axis_irq_enable_wr)
          axis_irq_enable <= axis_irq_wbit;
        if (axis_irq_clear_wr)
          axis_irq_pending <= 1'b0;
        if (desc64_axis_irq_pulse)
          axis_irq_pending <= 1'b1;
      end
    end

    // STOP for the cyclic AXI-Stream frontend is graceful in the desc64 frontend:
    // it stops new descriptor fetches and waits for already accepted work to
    // retire.  If the downstream audio FIFO has stopped accepting data, that
    // already accepted AXI-to-AXIS transfer can otherwise never retire.  Export
    // a stop-drain window so the top-level stream sink can accept and drop any
    // remaining beats until the frontend reports STOPPED.
    always_ff @(posedge clk_i or negedge rst_ni) begin
      if (!rst_ni) begin
        axis_stop_draining_q <= 1'b0;
      end else begin
        if (axis_cyclic_stopped)
          axis_stop_draining_q <= 1'b0;
        else if (axis_cyclic_stop_wr)
          axis_stop_draining_q <= 1'b1;
      end
    end

    assign axis_stop_draining = axis_stop_draining_q;
    assign axis_irq_o = axis_irq_pending && axis_irq_enable;

    // This is now coming in the request descriptor
    // always_comb begin
    ...
    assign axis_desc_is_stream = axis_desc_req.opt.dst_protocol == idma_pkg::AXI_STREAM;
    assign axis_desc_rounded_bytes =
        (({1'b0, axis_desc_req.length} + AxisBeatBytes - 1) / AxisBeatBytes) * AxisBeatBytes;
    assign axis_desc_required_bytes = axis_desc_rounded_bytes + AxisFifoSafetyMarginBytes;
    assign axis_fifo_free_bytes = AxisFifoCapacityBytes - axis_fifo_occupancy_i;
    assign axis_desc_reject = EnableAxisFifoAdmission && axis_desc_is_stream &&
        (axis_desc_required_bytes > AxisFifoCapacityBytes);
    assign axis_desc_admissible = !EnableAxisFifoAdmission || !axis_desc_is_stream ||
        ((axis_outstanding_q == '0) && !axis_stream_in_flight_q &&
         (axis_desc_required_bytes <= AxisFifoCapacityBytes) &&
    ...
    always_comb begin
      axis_reject_rsp = '0;
      axis_reject_rsp.last = 1'b1;
      axis_reject_rsp.error = 1'b1;
      axis_reject_rsp.pld.cause = axi_pkg::RESP_DECERR;
      axis_reject_rsp.pld.err_type = idma_pkg::BACKEND;
      axis_reject_rsp.pld.burst_addr = '0;
    end

    assign axis_desc_rsp = axis_reject_pending_q ?
        axis_reject_rsp : idma_rsp_fe[Desc64AxisIdx];
    assign axis_desc_rsp_valid = axis_reject_pending_q || idma_rsp_fe_valid[Desc64AxisIdx];
    assign idma_rsp_fe_ready[Desc64AxisIdx] = axis_desc_rsp_ready && !axis_reject_pending_q;
    assign axis_desc_accept = axis_desc_req_valid && axis_desc_req_ready && !axis_desc_reject;
    assign axis_desc_reject_accept = axis_desc_req_valid && axis_desc_req_ready && axis_desc_reject;
    assign axis_desc_backend_response = idma_rsp_fe_valid[Desc64AxisIdx] &&
        axis_desc_rsp_ready && !axis_reject_pending_q;

    always_ff @(posedge clk_i or negedge rst_ni) begin
      if (!rst_ni) begin
        axis_outstanding_q <= '0;
        axis_stream_in_flight_q <= 1'b0;
        axis_reject_pending_q <= 1'b0;
      end else begin
        unique case ({axis_desc_accept, axis_desc_backend_response})
          2'b10: axis_outstanding_q <= axis_outstanding_q + 1'b1;
          2'b01: axis_outstanding_q <= axis_outstanding_q - 1'b1;
          default: axis_outstanding_q <= axis_outstanding_q;
        endcase
        if (axis_desc_accept && axis_desc_is_stream)
          axis_stream_in_flight_q <= 1'b1;
        else if (axis_desc_backend_response && axis_stream_in_flight_q)
          axis_stream_in_flight_q <= 1'b0;

        if (axis_desc_reject_accept)
          axis_reject_pending_q <= 1'b1;
        else if (axis_reject_pending_q && axis_desc_rsp_ready)
          axis_reject_pending_q <= 1'b0;
      end
    end
    ...
    assign axis_stop_draining = 1'b0;
    assign axis_irq_o = 1'b0;
  end
  ...
  if (EnableReg64) begin : gen_reg64
    ...
    localparam logic [15:0] EngineStatusOffset = 16'h100;
  ...
  idma_axi_axis_fe_arb #(
    .NumFrontends  ( NumFrontends   ),
    .MaxOutstanding( DmaBackendDepth ),
    .idma_req_t    ( idma_req_t     ),
    .idma_rsp_t    ( idma_rsp_t     )
  ) fe_arb_i (
    .clk_i,
  ...
  // During cyclic STOP, consume any remaining AXI-Stream beats from the
  // backend and drop them.  This lets an already accepted AXI-to-AXIS transfer
  // retire even if the external audio FIFO is full or no longer being drained.
  always_comb begin
    axis_write_rsp = axis_write_rsp_i;
    if (axis_stop_draining)
      axis_write_rsp.tready = 1'b1;
  end

  always_comb begin
    axis_write_req_o = axis_write_req;
    if (axis_stop_draining)
      axis_write_req_o.tvalid = 1'b0;
  ...
  //--------------------------------------------------------------
    /*
    * Debug only: count sustained stalls at the three boundaries relevant
    * to AXI-memory -> AXI-Stream playback.
    *
    * These counters do not feed any functional signal.
    */
    // always_ff @(posedge clk_i or negedge rst_ni) begin
    //     if (!rst_ni) begin
    //         dbg_axis_stall_cycles   <= '0;
    //         dbg_axi_ar_stall_cycles <= '0;
    //         dbg_axi_r_stall_cycles  <= '0;
    //     end else begin
    //         /*
    //         * iDMA has a stream word available, but the I2S/AXIS path will not
    //         * accept it.
    //         */
    //         if (axis_write_req_o.valid && !axis_write_rsp_i.ready) begin
    //             if (!(&dbg_axis_stall_cycles))
    //             dbg_axis_stall_cycles <= dbg_axis_stall_cycles + 1'b1;
    //         end else begin
    //             dbg_axis_stall_cycles <= '0;
    //         end

    //         /*
    //         * iDMA wants to issue an AXI memory read, but the DDR/interconnect
    //         * side will not accept its address request.
    //         */
    //         if (axi_mst_be_req_o.ar_valid && !axi_mst_be_rsp_i.ar_ready) begin
    //             if (!(&dbg_axi_ar_stall_cycles))
    //             dbg_axi_ar_stall_cycles <= dbg_axi_ar_stall_cycles + 1'b1;
    //         end else begin
    //             dbg_axi_ar_stall_cycles <= '0;
    //         end

    //         /*
    //         * DDR presents read data, but the iDMA backend cannot accept it.
    //         * This is the signature expected if AXIS backpressure propagates
    //         * backwards into the AXI read-data channel.
    //         */
    //         if (axi_mst_be_rsp_i.r_valid && !axi_mst_be_req_o.r_ready) begin
    //             if (!(&dbg_axi_r_stall_cycles))
    //             dbg_axi_r_stall_cycles <= dbg_axi_r_stall_cycles + 1'b1;
    //         end else begin
    //             dbg_axi_r_stall_cycles <= '0;
    //         end
    //     end
...
module idma_axi_axis_fe_arb #(
  parameter int unsigned NumFrontends = 0,
  // Maximum number of requests the backend can accept without returning a response.
  ...
  // Include the maximum value itself; for DmaBackendDepth=260 this is 9 bits.
  localparam int unsigned OutstandingCountWidth =
      (MaxOutstanding > 0) ? unsigned'($clog2(MaxOutstanding + 1)) : 32'd1;
  ...
  (* mark_debug = "true" *) logic [OutstandingCountWidth-1:0] ongoing_req_cnt_d;
  (* mark_debug = "true" *) logic [OutstandingCountWidth-1:0] ongoing_req_cnt_q;
