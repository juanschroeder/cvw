...
`ifndef CVWSOC_CVA6_TYPEDEF_SVH_
`define CVWSOC_CVA6_TYPEDEF_SVH_
...
`define CVWSOC_CVA6_TYPEDEF_AXI_CT(__name, __addr_t, __id_t, __data_t, __strb_t, __user_t) \
  `AXI_TYPEDEF_ALL_CT(__name, __name``_req_t, __name``_rsp_t, \
      __addr_t, __id_t, __data_t, __strb_t, __user_t)

`define CVWSOC_CVA6_TYPEDEF_AXI(__name, __name_llc, __addr_t, __cfg) \
  localparam cvwsoc_cva6_pkg::axi_in_cvwsoc_t __name``__AxiIn = \
      cvwsoc_cva6_pkg::gen_cvwsoc_axi_in(__cfg); \
  ...
  `CVWSOC_CVA6_TYPEDEF_AXI_CT(__name``_mst, __addr_t, \
      __name``_mst_id_t, __name``_data_t, __name``_strb_t, __name``_user_t) \
  `CVWSOC_CVA6_TYPEDEF_AXI_CT(__name``_slv, __addr_t, \
      __name``_slv_id_t, __name``_data_t, __name``_strb_t, __name``_user_t) \
  `CVWSOC_CVA6_TYPEDEF_AXI_CT(__name``_llc, __addr_t, \
      __name``_llc_id_t, __name``_data_t, __name``_strb_t, __name``_user_t) \

`define CVWSOC_CVA6_TYPEDEF_REG(__name, __addr_t) \
  `REG_BUS_TYPEDEF_ALL(__name, __addr_t, logic [31:0], logic [3:0])

// REMOVED
// `define CVWSOC_CVA6_TYPEDEF_APB(__name, __addr_t) \
//   `APB_TYPEDEF_ALL(__name, __addr_t, logic [31:0], logic [3:0])

// Note that the prefix does *not* include a leading underscore.
`define CVWSOC_CVA6_TYPEDEF_ALL(__prefix, __cfg) \
  localparam type __prefix``addr_t = logic [__cfg.AddrWidth-1:0]; \
  `CVWSOC_CVA6_TYPEDEF_AXI(__prefix``axi, __prefix``axi_llc, __prefix``addr_t, __cfg) \
  `CVWSOC_CVA6_TYPEDEF_REG(__prefix``reg, __prefix``addr_t) //\
  // REMOVED
  //`CVWSOC_CVA6_TYPEDEF_APB(__prefix``apb, __prefix``addr_t)
