...
# 2) Wishbone LiteEth internal async PRE pins: already proved fanout only hits PRE pins.
...
# Ignore async CDC into first-stage synchronizer flops (LiteEth XilinxMultiReg stage0 + RMII reset stage0)
...
# AXI-LiteEth IRQ synchronizer (LiteEth clock -> bus clock).  AXI LiteEth is
# disabled for this Nexys configuration; retain this for when it is enabled.
...
# Old Xilinx/MMCM USB CDC hierarchy rules, kept for reference.
...
# ------------------------------------------------------------
# Generic OHCI CDC constraints (shared with the Genesys2 non-Xilinx build).
...
# Non-Xilinx AXI CDC constraints from cdc_fifo_gray.sv.
...
# ------------------------------------------------------------
# Remaining non-Xilinx CVWSoC timing constraints from the Genesys2 build.
# These peripherals are not connected/enabled on Nexys A7 yet.  Keep their
# exact constraints here, commented, so enabling a peripheral is deliberate.
